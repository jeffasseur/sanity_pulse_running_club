// Ignore TypeScript errors in this file, as it is a utility script that is not type-checked
export function initCountdown(){
  const reg={items:[],timer:null};
  ...
  function splitByUnits(ms: number, u: any){
    let secs=Math.max(0,Math.floor(ms/1000));
    const out={years:0,months:0,weeks:0,days:0,hours:0,minutes:0,seconds:0,done:ms<=0};
    const seq=[['years',31536000],['months',2592000],['weeks',604800],['days',86400],['hours',3600],['minutes',60],['seconds',1]];
    for(let i=0;i<seq.length;i++){
      const k=seq[i][0],len=seq[i][1];
      if(u[k]){ out[k]=Math.floor(secs/len); secs%=len; }
    }
    return out;
  }

  const sing={years:'year',months:'month',weeks:'week',days:'day',hours:'hour',minutes:'minute',seconds:'second'};
  const abbr={years:['yr.','yrs.'],months:['mo.','mos.'],weeks:['wk.','wks.'],days:['day','days'],hours:['hr.','hrs.'],minutes:['min.','mins.'],seconds:['sec.','secs.']};
  ...
  function make(root: HTMLElement){
    const u={}, order=['years','months','weeks','days','hours','minutes','seconds'];
    root.querySelectorAll('[data-countdown-update]').forEach(function(n){
      const k=(n.getAttribute('data-countdown-update')||'').toLowerCase();
      if(order.indexOf(k)>-1) u[k]=n;
    });
    const tgt=parseIso(root);
    if(tgt==null){
      root.setAttribute('data-countdown-status','error');
      let first=null; for(let i=0;i<order.length;i++){ if(u[order[i]]){ first=u[order[i]]; break; } }
      if(first) first.textContent='Invalid Date, use: 2026-03-21 11:36';
      order.forEach(function(k){ const n=u[k]; if(n&&n!==first) n.textContent=''; });
      return null;
    }
